...
/**
 * Builds a Docker image using a pre-determined version string.
 * Use this when you need multiple images to share the same version number.
 *
 * @param script The pipeline script context
 * @param version The version string to use (e.g., "2.3.45.213")
 * @param repository The Docker repository name
 * @param imageName The Docker image name
 * @param extraDockerBuildArguments Additional Docker build arguments
 * @param registryPublish Whether to push to registry
 * @param helmBuildChart Whether to build and push Helm chart
 * @param nugetPushOption NuGet publishing option
 * @param nugetPackageId NuGet package ID
 * @param dockerFileLocation Location of Dockerfile (default: './src')
 * @param overrideHelmDirectory Override Helm chart directory name
 * @param extraSHCommands Extra shell commands to run before build
 */
BuildInfo call(def script, String version, String repository, String imageName, String extraDockerBuildArguments, Boolean registryPublish, Boolean helmBuildChart = false, NugetPushOptionEnum nugetPushOption = NugetPushOptionEnum.NoPush, String nugetPackageId = '', String dockerFileLocation = './src', String overrideHelmDirectory = '', String extraSHCommands = '')
{
    BuildInfo buildInfo = new BuildInfo(steps, script)

    String label = new ContainerLabel("build", imageName).label

    // Use passed version instead of generating a new one
